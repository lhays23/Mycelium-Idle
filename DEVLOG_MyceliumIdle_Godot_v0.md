...
- Node tap open via `Area2D.input_event` still not firing reliably despite:
  - CollisionShape2D present
  - Pickable enabled
  - Object picking enabled in Project Settings
- Planned next fix:
  - Either switch to manual hit-test in `_input/_unhandled_input`
  - Or further debug picking layers/masks and event routing

---

## Next Steps
1) Finalize node tap behavior (reliable open of NodePanel)
2) Add basic node feedback (tap highlight / selected ring)

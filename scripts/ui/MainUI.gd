...
const MAP_PAN_THRESHOLD  := 12.0   # px moved before drag is considered a pan (touch-friendly)
...
var _vp_obtained_lbl: Label  = null       # obtained so far (live)
var _vp_timer_lbl: Label     = null       # time remaining (live)
var _vp_progress: ProgressBar = null      # delivery progress
var _vp_status_lbl: Label    = null       # "dissipated" notice
...
var _mc_owned_list: VBoxContainer = null
var _mc_gs_lbl: Label = null
# Mutation Chamber popups
var _mc_popup_layer: CanvasLayer = null
var _mc_popup_dimmer: ColorRect = null
var _mc_selected_mutation_id: String = ""
...
var _volatile_node_dot: Node2D = null   # visual on the map
...
func _ready() -> void:
	...
	_bind_volatile_panel()

	if game_state != null and game_state.has_signal("volatile_node_changed"):
		game_state.volatile_node_changed.connect(_on_volatile_node_changed)
		# Sync initial state — signal may have fired before we connected
		_refresh_volatile_node_visual()
...
func _theme_panel_shell(panel: Control) -> void:
	...
	sb_outer.content_margin_top    = 0
	sb_outer.content_margin_bottom = 0
	sb_outer.content_margin_left   = 0
	sb_outer.content_margin_right  = 0
...
func _theme_action_button(btn: Button) -> void:
	var tm := ThemeManager
	btn.focus_mode = Control.FOCUS_NONE   # prevent focus-grab on Android (requires 2 taps otherwise)
	var sb := StyleBoxFlat.new()
	...
	btn.add_theme_stylebox_override("focus", sb)   # no visible focus ring
...
func _process(dt: float) -> void:
	...
	if _ui_accum >= UI_REFRESH_DT:
		...
		# Live-update volatile node panel if open
		if volatile_panel != null and _open_panel == volatile_panel:
			_refresh_volatile_panel()

		# Live-update discovery popup cost labels and button
		_refresh_disc_popup_live()

		# Live-update refinery slot progress bars
		if _open_panel == refinery_panel:
			_update_refinery_slot_progress()
...
func _on_volatile_node_changed() -> void:
	_refresh_volatile_node_visual()


func _refresh_volatile_node_visual() -> void:
	if game_state == null or not game_state.has_method("get_volatile_node_ui"):
		return
	var ui: Dictionary = game_state.call("get_volatile_node_ui") as Dictionary
	var active: bool      = bool(ui.get("active", false))
	var delivering: bool  = bool(ui.get("delivering", false))
	var pos: Vector2      = ui.get("position", Vector2.ZERO) as Vector2

	# Remove old dot if node is no longer active
	if not active and not delivering:
		if _volatile_node_dot != null and is_instance_valid(_volatile_node_dot):
			_volatile_node_dot.queue_free()
			_volatile_node_dot = null
		return

	# Create dot if needed
	if _volatile_node_dot == null or not is_instance_valid(_volatile_node_dot):
		_volatile_node_dot = _make_volatile_node_dot()
		map_layer.add_child(_volatile_node_dot)

	# pos is offset from spore cloud center — same coordinate system as nodes
	_volatile_node_dot.position = spore_cloud.position + pos
	_volatile_node_dot.z_index  = 10
	_volatile_node_dot.visible  = true


func _make_volatile_node_dot() -> Node2D:
	var dot := Node2D.new()
	dot.name = "VolatileNodeDot"
	dot.z_index = 10  # Draw on top of everything

	# Bright yellow fill — unmissable for testing
	...
	poly.polygon = points
	poly.color   = Color(1.0, 1.0, 0.0, 1.0)  # Bright yellow
	dot.add_child(poly)
...
func _bind_digest_panel() -> void:
	...
	# ── Root Control (explicit positioning via _layout_digest_panel) ──────
	...
	top_vbox.add_child(_digest_header)

	# ── Tab row ───────────────────────────────────────────────────────────
	digest_tabs_row = HBoxContainer.new()
	digest_tabs_row.add_theme_constant_override("separation", 4)
	digest_tabs_row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	top_vbox.add_child(digest_tabs_row)
	...
	# Enforce equal fixed width after layout
	await get_tree().process_frame
	if is_instance_valid(digest_tabs_row):
		var avail := digest_tabs_row.size.x - 4.0 * 2.0
		var btn_w  := floorf(avail / 3.0)
		for child in digest_tabs_row.get_children():
			if child is Button:
	...
	_digest_bottom_bar.name = "DigestBottomBar"
...
func _layout_digest_panel() -> void:
	...
	# Top pad: header + tabs — let it size naturally, then read its height
	var top_pad := root.find_child("DigestTopPad", true, false) as Control
	var top_h := 0.0
	if top_pad != null:
		top_pad.position = Vector2.ZERO
		top_pad.size     = Vector2(W, 0)  # height auto from contents
		await get_tree().process_frame
		top_h = top_pad.size.y
		top_pad.size = Vector2(W, top_h)

	if _digest_scroll != null:
		_digest_scroll.position = Vector2(0, top_h)
		_digest_scroll.size     = Vector2(W, H - top_h - BAR_H)
...
func _get_refinery_inventory_signature() -> String:
	# Only track UNLOCK state, not live resource amounts.
	# Amounts changing every tick would cause constant rebuilds and break button clicks.
	var parts: Array[String] = []
	if game_state == null:
		return ""

	# Which compounds are unlocked
	if game_state.has_method("get_unlocked_compound_ids"):
		var unlocked: Array = game_state.call("get_unlocked_compound_ids") as Array
		for id in unlocked:
			parts.append("cu:%s" % str(id))
	else:
		var compound_defs: Dictionary = game_state.get("compound_defs")
		for id_v in compound_defs.keys():
			var id := str(id_v)
			var paid: Dictionary = game_state.get("paid_compound_unlocks")
			if bool(paid.get(id, false)):
				parts.append("cu:%s" % id)

	# Which solutions are unlocked
	if game_state.has_method("get_unlocked_solution_ids"):
		var unlocked: Array = game_state.call("get_unlocked_solution_ids") as Array
		for id in unlocked:
			parts.append("su:%s" % str(id))
	else:
		var solution_defs: Dictionary = game_state.get("solution_defs")
		for id_v in solution_defs.keys():
			var id := str(id_v)
			var paid: Dictionary = game_state.get("paid_solution_unlocks")
			if bool(paid.get(id, false)):
				parts.append("su:%s" % id)

	# Active slot counts
	var r_slots: int = int(game_state.get("unlocked_refinery_slots"))
	var s_slots: int = int(game_state.get("unlocked_synth_slots"))
	parts.append("rs:%d" % r_slots)
	parts.append("ss:%d" % s_slots)

	# Mutagen lab discovery
	if game_state.has_method("is_mutagen_lab_unlocked"):
		parts.append("ml:%s" % str(bool(game_state.call("is_mutagen_lab_unlocked"))))
...
func _get_discovery_signature() -> String:
	# Only track discovery unlock/level state — NOT live resource amounts.
	# Resources change every tick and would cause constant panel rebuilds.
	var parts: Array[String] = []
	if game_state == null:
		return ""
...
func _bind_discoveries_panel() -> void:
	...
	grid.custom_minimum_size = Vector2(0, 1200)
...
func _refresh_discoveries_panel() -> void:
	...
	var positions: Dictionary = {
		# Core tree
		"mycelial_insight":      Vector2(col_c, row[1]),
		"aura_activation":       Vector2(col_c, row[0]),
		...
		"volatile_nodes":        Vector2(col_r, row[3]),
		"volatile_magnitude":    Vector2(col_r, row[4]),
		# Mutagen chain — left column from synthesis down
	...
	var connections: Array = [
		...
		# Volatile chain
		["aura_activation",  "volatile_nodes"],
		["volatile_nodes",   "volatile_magnitude"],
		# Synth → Mutagen chain
		["synthesis",        "mutagen_lab"],
		["mutagen_lab",      "strain_t2"],
		["strain_t2",        "strain_t3"],
		["strain_t3",        "strain_t4"],
		["strain_t3",        "mutagen_lab_slot2"],
		["strain_t4",        "strain_t5"],
		["strain_t5",        "strain_t6"],
		["strain_t5",        "mutagen_lab_slot3"],
	]
...
func _show_disc_popup(disc_id: String, entry: Dictionary) -> void:
	...
	_disc_popup_cost_labels = []
	_disc_popup_buy_btn = null
	var vp := get_viewport_rect().size
	var tm := ThemeManager
	...
	if complete and max_level <= 1:
		...
		if typeof(costs_v) == TYPE_ARRAY:
			...
			for cost_v in costs:
				...
				_disc_popup_cost_labels.append({"res_id": res_id, "qty": qty, "lbl": amt_lbl})
...
func _refresh_disc_popup_live() -> void:
	if _disc_popup_layer == null or not is_instance_valid(_disc_popup_layer):
		return
	if _disc_popup_disc_id == "" or game_state == null:
		return
	var tm := ThemeManager

	# Update cost labels
	for entry_v in _disc_popup_cost_labels:
		var entry: Dictionary = entry_v as Dictionary
		var res_id: String = str(entry.get("res_id", ""))
		var qty: int       = int(entry.get("qty", 0))
		var lbl: Label     = entry.get("lbl", null) as Label
		if lbl == null or not is_instance_valid(lbl):
			continue
		var have: int = int(game_state.call("get_amount", res_id))
		var can_afford: bool = have >= qty
		lbl.text = "%s / %s" % [_fmt_int(have), _fmt_int(qty)]
		lbl.add_theme_color_override("font_color",
			tm.c("accent") if can_afford else tm.c("text_muted"))

	# Update buy button enabled state
	if _disc_popup_buy_btn != null and is_instance_valid(_disc_popup_buy_btn):
		var check_v = game_state.call("can_buy_discovery", _disc_popup_disc_id)
		if typeof(check_v) == TYPE_DICTIONARY:
			_disc_popup_buy_btn.disabled = not bool((check_v as Dictionary).get("ok", false))


# ── Volatile Node Panel ────────────────────────────────────────────────────────

func _bind_volatile_panel() -> void:
	if _panel_container == null:
		return
	var tm := ThemeManager

	var panel := PanelContainer.new()
	panel.name = "VolatilePanel"
	panel.visible = false
	...
	pad_vbox.add_theme_constant_override("separation", 12)
	pad.add_child(pad_vbox)

	# Header row
	var hdr := HBoxContainer.new()
	pad_vbox.add_child(hdr)

	var title := Label.new()
	title.text = "Volatile Node"
	...
	close_btn.add_theme_color_override("font_color", tm.c("text_muted"))
	...
	# Resource name
	_vp_res_lbl = Label.new()
	_vp_res_lbl.add_theme_font_size_override("font_size", 15)
	_vp_res_lbl.add_theme_color_override("font_color", tm.c("text_primary"))
	pad_vbox.add_child(_vp_res_lbl)

	# Total yield row
	var total_row := HBoxContainer.new()
	var total_l := Label.new()
	total_l.text = "Total yield"
	total_l.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	total_l.add_theme_font_size_override("font_size", 13)
	total_l.add_theme_color_override("font_color", tm.c("text_secondary"))
	total_row.add_child(total_l)
	_vp_total_lbl = Label.new()
	_vp_total_lbl.add_theme_font_size_override("font_size", 13)
	_vp_total_lbl.add_theme_color_override("font_color", tm.c("text_primary"))
	total_row.add_child(_vp_total_lbl)
	pad_vbox.add_child(total_row)

	# Obtained row (live)
	var obt_row := HBoxContainer.new()
	var obt_l := Label.new()
	obt_l.text = "Obtained"
	obt_l.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	obt_l.add_theme_font_size_override("font_size", 13)
	obt_l.add_theme_color_override("font_color", tm.c("text_secondary"))
	obt_row.add_child(obt_l)
	_vp_obtained_lbl = Label.new()
	_vp_obtained_lbl.add_theme_font_size_override("font_size", 13)
	_vp_obtained_lbl.add_theme_color_override("font_color", tm.c("accent"))
	obt_row.add_child(_vp_obtained_lbl)
	pad_vbox.add_child(obt_row)

	# Time remaining row (live)
	var time_row := HBoxContainer.new()
	var time_l := Label.new()
	time_l.text = "Time remaining"
	time_l.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	time_l.add_theme_font_size_override("font_size", 13)
	time_l.add_theme_color_override("font_color", tm.c("text_secondary"))
	time_row.add_child(time_l)
	_vp_timer_lbl = Label.new()
	_vp_timer_lbl.add_theme_font_size_override("font_size", 13)
	_vp_timer_lbl.add_theme_color_override("font_color", tm.c("text_primary"))
	time_row.add_child(_vp_timer_lbl)
	pad_vbox.add_child(time_row)

	# Progress bar
	_vp_progress = ProgressBar.new()
	_vp_progress.min_value = 0.0
	_vp_progress.max_value = 100.0
	_vp_progress.value     = 0.0
	_vp_progress.show_percentage = false
	_vp_progress.custom_minimum_size = Vector2(0, 10)
	pad_vbox.add_child(_vp_progress)

	# Status label (shown after dissipation)
	_vp_status_lbl = Label.new()
	_vp_status_lbl.text = ""
	_vp_status_lbl.add_theme_font_size_override("font_size", 13)
	_vp_status_lbl.add_theme_color_override("font_color", tm.c("text_muted"))
	_vp_status_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_vp_status_lbl.visible = false
	pad_vbox.add_child(_vp_status_lbl)


func _refresh_volatile_panel() -> void:
	if game_state == null or not game_state.has_method("get_volatile_node_ui"):
		return
	var ui: Dictionary = game_state.call("get_volatile_node_ui") as Dictionary
	...
	var active:     bool  = bool(ui.get("active", false))
	var delivering: bool  = bool(ui.get("delivering", false))
	var pct:        int   = int(ui.get("delivery_pct", 0))
	var rate:       float = float(ui.get("rate_per_sec", 0.0))
	var total:      float = rate * 60.0
	var obtained:   float = total * float(pct) / 100.0
	var secs_left:  float = 60.0 * (1.0 - float(pct) / 100.0)
	var res_name:   String = str(ui.get("resource_name", ""))

	if _vp_res_lbl != null:
		_vp_res_lbl.text = res_name if res_name != "" else "Volatile Node"
	if _vp_total_lbl != null:
		_vp_total_lbl.text = _fmt_num(int(total))
	if _vp_obtained_lbl != null:
		_vp_obtained_lbl.text = _fmt_num(int(obtained))
	if _vp_timer_lbl != null:
		_vp_timer_lbl.text = "%.0fs" % maxf(0.0, secs_left) if (active or delivering) else "—"
	if _vp_progress != null:
		_vp_progress.value = float(pct)

	# Show dissipated notice when neither active nor delivering
	var dissipated: bool = not active and not delivering and total > 0.0
	if _vp_status_lbl != null:
		_vp_status_lbl.visible = dissipated
		if dissipated:
			_vp_status_lbl.text = "The volatile node has dissipated."


func _show_volatile_panel() -> void:
	if volatile_panel == null or not is_instance_valid(volatile_panel):
		return
	_toggle_panel(volatile_panel)
...
func _bind_refinery_panel() -> void:
	...
	_refinery_header.add_theme_color_override("font_color", tm.c("accent"))
	top_vbox.add_child(_refinery_header)

	refinery_tabs_row = HBoxContainer.new()
	refinery_tabs_row.name = "RefineryTabsRow"
	refinery_tabs_row.add_theme_constant_override("separation", 4)
	refinery_tabs_row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	top_vbox.add_child(refinery_tabs_row)

	for tab_data in [
		["Compounds", "compound",  refinery_tab_compounds],
		["Solutions",  "solution", refinery_tab_solutions],
		["Mutagens",   "mutagen",  refinery_tab_mutagens],
	]:
		var btn := Button.new()
		btn.text = tab_data[0]
		btn.focus_mode = Control.FOCUS_NONE
		...
		btn.clip_text = true
		btn.add_theme_font_size_override("font_size", 11)
		var cat: String = tab_data[1]
		btn.pressed.connect(func(): _set_refinery_active_category(cat))
		refinery_tabs_row.add_child(btn)
		match tab_data[1]:
			"compound": refinery_tab_compounds = btn
	...
	if _refinery_active_category == "compound":
		if not game_state.has_method("is_refinery_unlocked") or not bool(game_state.call("is_refinery_unlocked")):
			if refinery_feedback != null:
				...
				refinery_feedback.visible = true
			return
		...
		if not visible_unlock_ids.is_empty():
			...
			recipe_header.add_theme_font_size_override("font_size", 13)
			recipe_header.add_theme_color_override("font_color", ThemeManager.c("text_secondary"))
...
func _build_mutagen_lab_tab() -> void:
	...
	var available_tiers: Array     = data.get("available_tiers", []) as Array
	...
	for i in range(slot_count):
		...
		refinery_list.add_child(_make_mutagen_slot_card(i, slot_data, available_tiers))
...
func _make_slot_row(entry: Dictionary, category: String) -> Control:
	...
	if not is_idle:
		...
		fill.name = "SlotFill_%s_%d" % [category, slot_number]
		fill.set_meta("slot_number", slot_number)
		fill.set_meta("category", category)
...
func _on_settings_new_game_pressed() -> void:
	if _settings_confirm_action != "new_game":
		...
		if settings_feedback != null:
			settings_feedback.text = "Press New Game again to confirm. ALL progress including mutations will be erased."
		_refresh_settings_panel()
		return

	_clear_settings_confirmation()

	if game_state == null or not game_state.has_method("reset_all_progress"):
		return

	game_state.call("reset_all_progress")
	_handle_runtime_state_reload()

	if settings_feedback != null:
		settings_feedback.text = "New game started."
	_refresh_settings_panel()
...
func _all_panels() -> Array[Control]:
	var panels: Array[Control] = [upgrades_panel, discoveries_panel, refinery_panel, digest_panel, settings_panel, node_panel, mutation_chamber_panel]
	if volatile_panel != null and is_instance_valid(volatile_panel):
		panels.append(volatile_panel)
	return panels
...
func _open(panel: Control) -> void:
	...
	if volatile_panel != null and panel == volatile_panel:
		_refresh_volatile_panel()
...
func _input(event: InputEvent) -> void:
	# Volatile node tap — check before anything else
	if _volatile_node_dot != null and is_instance_valid(_volatile_node_dot):
		var is_tap := false
		...
		if event is InputEventScreenTouch and (event as InputEventScreenTouch).pressed:
			...
			is_tap = true
		if is_tap and _volatile_node_dot != null and is_instance_valid(_volatile_node_dot):
			var dot_screen: Vector2 = get_viewport().get_screen_transform() * _volatile_node_dot.global_position
			if tap_pos.distance_to(dot_screen) <= 32.0 * _map_zoom:
				if game_state != null and game_state.has_method("tap_volatile_node"):
					game_state.call("tap_volatile_node")
				_show_volatile_panel()
				get_viewport().set_input_as_handled()
				return
	...
	if _open_panel != null and _open_panel != node_panel:
		if _open_panel == discoveries_panel:
			...
			if is_press_start:
				...
				# Don't return — let the Button receive the press too
				return
			...
			if is_press_end:
				...
				# If we didn't drag, let the event fall through so buttons fire
				if _disc_drag_has_moved:
					return
...
func _bind_mutation_chamber_panel() -> void:
	...
	top_pad.add_theme_constant_override("margin_bottom", 6)
	root.add_child(top_pad)

	var top_hbox := HBoxContainer.new()
	top_pad.add_child(top_hbox)

	var lbl_title := Label.new()
	lbl_title.text = "Mutation Chamber"
	lbl_title.add_theme_font_size_override("font_size", 17)
	lbl_title.add_theme_color_override("font_color", tm.c("accent"))
	...
	_mc_gs_lbl.add_theme_font_size_override("font_size", 15)
	_mc_gs_lbl.add_theme_color_override("font_color", tm.c("accent"))
	top_hbox.add_child(_mc_gs_lbl)

	# ── Two action buttons ────────────────────────────────────────────────────
	var btn_pad := MarginContainer.new()
	btn_pad.add_theme_constant_override("margin_left",   16)
	btn_pad.add_theme_constant_override("margin_right",  16)
	btn_pad.add_theme_constant_override("margin_bottom",  8)
	root.add_child(btn_pad)

	var btn_row := HBoxContainer.new()
	btn_row.add_theme_constant_override("separation", 10)
	btn_pad.add_child(btn_row)

	var btn_mutate := Button.new()
	btn_mutate.name = "BtnMutate"
	btn_mutate.text = "MUTATE"
	btn_mutate.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_mutate.custom_minimum_size = Vector2(0, 44)
	btn_mutate.focus_mode = Control.FOCUS_NONE
	_theme_action_button(btn_mutate)
	btn_mutate.pressed.connect(_show_mutate_popup)
	btn_row.add_child(btn_mutate)

	var btn_mutations := Button.new()
	btn_mutations.name = "BtnMutations"
	btn_mutations.text = "MUTATIONS"
	btn_mutations.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_mutations.custom_minimum_size = Vector2(0, 44)
	btn_mutations.focus_mode = Control.FOCUS_NONE
	_theme_action_button(btn_mutations)
	btn_mutations.pressed.connect(_show_mutations_popup)
	btn_row.add_child(btn_mutations)

	# ── Separator + section label ─────────────────────────────────────────────
	root.add_child(HSeparator.new())

	var owned_hdr_pad := MarginContainer.new()
	owned_hdr_pad.add_theme_constant_override("margin_left",  16)
	owned_hdr_pad.add_theme_constant_override("margin_top",    8)
	owned_hdr_pad.add_theme_constant_override("margin_bottom", 4)
	root.add_child(owned_hdr_pad)

	var owned_hdr := Label.new()
	owned_hdr.text = "YOUR MUTATIONS"
	owned_hdr.add_theme_font_size_override("font_size", 10)
	owned_hdr.add_theme_color_override("font_color", tm.c("text_muted"))
	owned_hdr_pad.add_child(owned_hdr)

	# ── Owned mutations scroll list ───────────────────────────────────────────
	var scroll := ScrollContainer.new()
	...
	_mc_owned_list = VBoxContainer.new()
	_mc_owned_list.name = "OwnedList"
	_mc_owned_list.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_mc_owned_list.add_theme_constant_override("separation", 4)
	scroll.add_child(_mc_owned_list)
...
func _refresh_mutation_chamber_panel() -> void:
	...
	var btn_mutate: Button = mutation_chamber_panel.find_child("BtnMutate", true, false) as Button
	if btn_mutate != null:
		btn_mutate.disabled = not bool(data.get("can_prestige", false))

	if _mc_owned_list == null:
		return
	for child in _mc_owned_list.get_children():
		child.queue_free()
	...
	for mut_v in mutations:
		...
		if bool(m.get("is_owned", false)) and not bool(m.get("is_reserved", false)):
			_mc_owned_list.add_child(_make_owned_mutation_row(m))
			any_owned = true
	...
	if not any_owned:
		...
		var lbl := Label.new()
		lbl.text = "No mutations unlocked yet.\nTap MUTATIONS to spend Genetic Strain."
		lbl.add_theme_font_size_override("font_size", 13)
		lbl.add_theme_color_override("font_color", ThemeManager.c("text_muted"))
		lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		pad.add_child(lbl)
		_mc_owned_list.add_child(pad)


func _make_owned_mutation_row(m: Dictionary) -> Control:
	var tm := ThemeManager
	...
	var buff_desc: String = str(m.get("buff_description", ""))
	...
	var lbl_name := Label.new()
	lbl_name.text = str(m.get("name", mid))
	lbl_name.add_theme_font_size_override("font_size", 14)
	...
	lbl_lv.add_theme_color_override("font_color", tm.c("text_muted"))
	name_row.add_child(lbl_lv)
	info.add_child(name_row)

	var lbl_buff := Label.new()
	lbl_buff.text = buff_desc
	lbl_buff.add_theme_font_size_override("font_size", 12)
	lbl_buff.add_theme_color_override("font_color", tm.c("accent_dim"))
	info.add_child(lbl_buff)
	hbox.add_child(info)
...
func _show_mutate_popup() -> void:
	if game_state == null or not game_state.has_method("get_mutation_chamber_ui"):
		return
	_close_mc_popup()
	...
	if typeof(data_v) != TYPE_DICTIONARY:
		return
	var data: Dictionary = data_v as Dictionary
	var breakdown: Dictionary = data.get("mutate_breakdown", {}) as Dictionary
	var tm := ThemeManager
	var vp := get_viewport_rect().size

	_mc_popup_layer = CanvasLayer.new()
	_mc_popup_layer.layer = 10
	add_child(_mc_popup_layer)

	_mc_popup_dimmer = ColorRect.new()
	_mc_popup_dimmer.color = Color(0, 0, 0, 0)
	_mc_popup_dimmer.set_anchors_preset(Control.PRESET_FULL_RECT)
	_mc_popup_dimmer.mouse_filter = Control.MOUSE_FILTER_STOP
	_mc_popup_dimmer.gui_input.connect(func(e: InputEvent):
		if (e is InputEventMouseButton or e is InputEventScreenTouch) and e.is_pressed():
			_close_mc_popup()
	)
	_mc_popup_layer.add_child(_mc_popup_dimmer)

	var popup := PanelContainer.new()
	popup.custom_minimum_size = Vector2(vp.x * 0.82, 0)
	var sb_popup := StyleBoxFlat.new()
	sb_popup.bg_color     = tm.c("bg_deep")
	sb_popup.border_color = tm.c("accent_border")
	sb_popup.set_border_width_all(1)
	sb_popup.set_corner_radius_all(14)
	sb_popup.content_margin_left   = 20
	sb_popup.content_margin_right  = 20
	sb_popup.content_margin_top    = 18
	sb_popup.content_margin_bottom = 18
	popup.add_theme_stylebox_override("panel", sb_popup)
	_mc_popup_layer.add_child(popup)
	...
	hdr.add_theme_color_override("font_color", tm.c("accent"))
	vbox.add_child(hdr)

	var desc := Label.new()
	desc.text = "Convert all progress and start anew\nin exchange for Genetic Strain."
	desc.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	desc.add_theme_font_size_override("font_size", 12)
	desc.add_theme_color_override("font_color", tm.c("text_muted"))
	...
	var rows := [
		["Nutrient Value",       _fmt_num(int(breakdown.get("nutrient_value", 0)))],
		["Base Reward",          "+%s GS" % _fmt_int(int(breakdown.get("base_gs", 0)))],
		["Mutagen Reward",       "+%s GS" % _fmt_int(int(breakdown.get("mutagen_reward", 0)))],
		["Strain Harvest (M09)", "+%s GS" % _fmt_int(int(breakdown.get("strain_harvest", 0)))],
		["Mutagen Yield (M13)",  "included in Mutagen Reward"],
	]
	for row_data in rows:
		var row := HBoxContainer.new()
		var lbl_key := Label.new()
		lbl_key.text = str(row_data[0])
		lbl_key.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		lbl_key.add_theme_font_size_override("font_size", 13)
		lbl_key.add_theme_color_override("font_color", tm.c("text_secondary"))
		row.add_child(lbl_key)
		var lbl_val := Label.new()
		lbl_val.text = str(row_data[1])
		lbl_val.add_theme_font_size_override("font_size", 13)
		lbl_val.add_theme_color_override("font_color", tm.c("text_primary"))
	...
	var total_row := HBoxContainer.new()
	var lbl_total_key := Label.new()
	lbl_total_key.text = "Total GS"
	lbl_total_key.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	lbl_total_key.add_theme_font_size_override("font_size", 16)
	lbl_total_key.add_theme_color_override("font_color", tm.c("accent"))
	total_row.add_child(lbl_total_key)
	var lbl_total_val := Label.new()
	lbl_total_val.text = "+%s GS" % _fmt_int(int(breakdown.get("total_gs", 0)))
	lbl_total_val.add_theme_font_size_override("font_size", 16)
	lbl_total_val.add_theme_color_override("font_color", tm.c("accent"))
	total_row.add_child(lbl_total_val)
	vbox.add_child(total_row)

	var confirm_btn := Button.new()
	confirm_btn.text = "MUTATE"
	confirm_btn.custom_minimum_size = Vector2(0, 44)
	confirm_btn.disabled = not bool(data.get("can_prestige", false))
	confirm_btn.focus_mode = Control.FOCUS_NONE
	_theme_action_button(confirm_btn)
	confirm_btn.pressed.connect(_on_mutate_confirm_pressed)
...
func _show_mutations_popup() -> void:
	...
	var gs_balance: int  = int(data.get("gs_balance", 0))
	var tm := ThemeManager
	var vp := get_viewport_rect().size
	...
	for mut_v in mutations:
		var m: Dictionary = mut_v as Dictionary
		if bool(m.get("is_available_to_unlock", false)) and not bool(m.get("is_reserved", false)):
			available.append(m)
	...
	gs_lbl.text = "Balance: %s GS" % _fmt_int(gs_balance)
	gs_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	gs_lbl.add_theme_font_size_override("font_size", 13)
	gs_lbl.add_theme_color_override("font_color", tm.c("text_secondary"))
	vbox.add_child(gs_lbl)

	if available.is_empty():
		var lbl := Label.new()
		lbl.text = "All mutations unlocked!"
		lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		...
		for m_v in available:
			var m: Dictionary = m_v as Dictionary
			cards_vbox.add_child(_make_mutation_select_card(m, unlock_cost, cards_vbox))

	vbox.add_child(HSeparator.new())

	var confirm_btn := Button.new()
	confirm_btn.name = "MutationConfirmBtn"
	confirm_btn.text = "Unlock  —  %s GS" % _fmt_int(unlock_cost)
	confirm_btn.custom_minimum_size = Vector2(0, 44)
	confirm_btn.disabled = true
	confirm_btn.focus_mode = Control.FOCUS_NONE
	_theme_action_button(confirm_btn)
	confirm_btn.pressed.connect(func():
		if _mc_selected_mutation_id != "":
			_on_mutation_unlock_pressed(_mc_selected_mutation_id)
	)
	vbox.add_child(confirm_btn)
...
func _make_mutation_select_card(m: Dictionary, unlock_cost: int, cards_parent: VBoxContainer) -> Control:
	...
	sb_card.border_color = tm.c("accent_dim") if can_unlock else tm.c("border")
	...
	lbl_id.add_theme_color_override("font_color", tm.c("text_muted"))
	name_row.add_child(lbl_id)
	name_row.add_child(Control.new())  # spacer
	(name_row.get_child(1) as Control).size_flags_horizontal = Control.SIZE_EXPAND_FILL
	...
	lbl_name.add_theme_color_override("font_color",
		tm.c("text_primary") if can_unlock else tm.c("text_muted"))
	vbox.add_child(lbl_name)

	var lbl_desc := Label.new()
	lbl_desc.text = str(m.get("buff_description", str(m.get("desc", ""))))
	...
	card.gui_input.connect(func(e: InputEvent):
		if not can_unlock:
			return
		if (e is InputEventMouseButton and e.button_index == MOUSE_BUTTON_LEFT and e.pressed) or \
		   (e is InputEventScreenTouch and e.pressed):
			_mc_selected_mutation_id = mid
			for sibling in cards_parent.get_children():
				if not is_instance_valid(sibling):
					continue
				var is_sel: bool = sibling.name == "Card_" + mid
				var sb_upd := StyleBoxFlat.new()
				sb_upd.set_corner_radius_all(8)
				sb_upd.set_border_width_all(1)
				sb_upd.content_margin_left   = 12
				sb_upd.content_margin_right  = 12
				sb_upd.content_margin_top    = 10
				sb_upd.content_margin_bottom = 10
				if is_sel:
					sb_upd.bg_color     = tm.c("bg_row")
					sb_upd.border_color = tm.c("accent")
					sb_upd.border_width_left = 3
				else:
					sb_upd.bg_color     = tm.c("bg_panel")
					sb_upd.border_color = tm.c("accent_dim")
				(sibling as PanelContainer).add_theme_stylebox_override("panel", sb_upd)
			var popup_node: Control = _mc_popup_layer.get_node_or_null("MutationsPopup") as Control
			if popup_node != null:
				var confirm: Button = popup_node.find_child("MutationConfirmBtn", true, false) as Button
				if confirm != null:
...
func _close_mc_popup() -> void:
	_mc_selected_mutation_id = ""
	if _mc_popup_layer != null and is_instance_valid(_mc_popup_layer):
		_mc_popup_layer.queue_free()
	_mc_popup_layer  = null
	_mc_popup_dimmer = null
...
func _on_mutate_confirm_pressed() -> void:
